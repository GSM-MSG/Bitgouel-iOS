...
func registerModulePaths() {
    updateFileContent(
        ...
        finding: "enum \(layer.rawValue): String, TMATargetPathConvertable {\n",
...
func makeScaffold(target: TMATargetType) {
    _ = try? bash.run(
        commandName: "tuist",
...
let hasTests = readLine()?.lowercased() == "y"
