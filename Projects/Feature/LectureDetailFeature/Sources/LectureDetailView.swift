...
struct LectureDetailView: View {
    @StateObject var viewModel: LectureDetailViewModel
                                ...
                                LazyVStack(alignment: .leading, spacing: 16) {
                                    ForEach(lectureDetail.lectureDates, id: \.completeDate) { lectureDate in
                                        HStack(spacing: 0) {
                                            ...
                                            Text(
                                                "\(lectureDate.completeDate.toStringCustomFormat(format: "yyyy년 MM월 dd일"))"
                                            )
                                            // swiftlint: enable line_length

                                            Text("\(lectureDate.startTime.toStringCustomFormat(format: "H시 m분"))")

                                            Text(" ~")

                                            Text("\(lectureDate.endTime.toStringCustomFormat(format: "H시m분"))")
                                        }
                                    }
                                    .bitgouelFont(.text3, color: .greyscale(.g4))
                                }
